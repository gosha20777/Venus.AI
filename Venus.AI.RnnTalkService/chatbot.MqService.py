...
def main():
    ...
    global net, sess, chars, vocab, max_length, beam_width, relevance, temperature, topn, max_replic_count
    net, sess, chars, vocab, max_length, beam_width, relevance, temperature, topn, max_replic_count = initializeRnn(args)
    ...
    channel.queue_declare(queue='RnnTalkService_input_ru')

    print('[*] Waiting for messages. To exit press CTRL+C')

    channel.basic_consume(callback,
                      queue='RnnTalkService_input_ru',
                      no_ack=True)
...
def callback(ch, method, properties, body):
    ...
    user_input = message['textData']
    uid = message['id']
    states_bin = message['talkContext']
    replic_count = message['talkReplicCount']

    if(states_bin == 'null' or states_bin == None or len(states_bin) < 10):
        states = None
    else:
        states_bin_bytes = base64.decodestring(states_bin.encode('utf8'))
        states = pickle.loads(states_bin_bytes)
        pass
    
    print("[*] recive message:\n\ttextData: ",user_input,"\n\tid: ",uid)

    #states, replic_count = restoreUserConversationState(uid)
    output_text, states, replic_count = invorkRnn(net, sess, chars, vocab, max_length, beam_width, relevance, temperature, topn, max_replic_count, states, replic_count, user_input)
    #saveUserConversationState(uid, states, replic_count)
    
    states_bin_bytes = pickle.dumps(states, 2)
    states_bin = base64.encodestring(states_bin_bytes).decode()

    data = {"id": uid, "textData": output_text, "talkContext": states_bin, "talkReplicCount": replic_count}

    connection = pika.BlockingConnection(pika.ConnectionParameters(
        host='localhost'))
    channel = connection.channel()
    channel.queue_declare(queue='RnnTalkService_output_ru')
    body = json.dumps(data)
    channel.basic_publish(exchange='',
                      routing_key='RnnTalkService_output_ru',
                      body=body)
    print("[*] Sent message:\n\ttextData: ",output_text,"\n\tid: ",uid)
    connection.close()

def initializeRnn(args):
    # инициализировать нейросеть в зависимости от конфигурационного файлв "config.json"
    # файл конфигурации содержит поля:
    #   net_path         = путь до файла с моделью
    #   chars            = набор символов
    #   vocab            = словарь
    #   max_length       = длинну фраз
    #   beam_width       = ширину луча для посика
    #   relevance        = каоэффициент прдсказания
    #   topn             = коаэффициент вероятности для выбора следующего символа в слове
    #   max_replic_count = максимальное число реплик в диалоге
    # вернуть эти параметры заменив пути на обьекты
    ...
    max_replic_count = 20
    return net, sess, chars, vocab, args.n, args.beam_width, args.relevance, args.temperature, args.topn, max_replic_count
    
def restoreUserConversationState(id):
    # обратиться к бд.
    # достать из нее состояние (states) диалога пользователя и колличество реплик в диалоге
    # если колличество реплик = 0 ИЛИ такого id не сужествует в бд
    #   инициализировать состояние (states) началтным значениям None
    # вернуть состояние (states), колличество реплик (replic_count).
    ### p.s не забыть проверить состояние (states) на null подумать об исключениях

    replic_count = 0
    states = None    
    return states, replic_count

def saveUserConversationState(id, states, replic_count):
    # обратиться к бд.
    # обновить данные о пользователе или добавить нового.
    ### p.s подумать об исключениях
    return

def invorkRnn(net, sess, chars, vocab, max_length, beam_width, relevance, temperature, topn, max_replic_count, states, replic_count, user_input):
    # получает на вход
    #  Глобальные переменные (одинаковые для кадлой сессии):
    #   sess             = сессия
    #   net              = модель
    #   chars            = набор символов
    #   vocab            = словарь
    ...
    #   topn             = коаэффициент вероятности для выбора следующего символа в слове
    #   max_replic_count = максимальное число реплик в диалоге
    #  Локальные переменные (ращные для каджого запроса):
    #   states           = состояние диалога
    #   replic_count     = кол-во реплик в диалоге
    #   user_input       = реплика пользователя
    #   temperature      = тональность реплики пользователя
    # генерирует ответ
    # увеличивает replic_count на 1 по модулю max_replic_count
    # возвращает output_text states, replic_count
    ...
    if not user_command_entered:
        states = forward_text(net,
         sess, 
         states, 
         relevance,
         vocab,
         sanitize_text(vocab, "> " + user_input + "\n>"))
        ...
        for i, char_token in enumerate(computer_response_generator):
            ...
            output_text += possibly_escaped_char(out_chars)
        ...
        replic_count = replic_count + 1 % max_replic_count
    return output_text, states, replic_count
